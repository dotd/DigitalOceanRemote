...
from DigitalOceanRemote.definitions import PROJECT_ROOT_DIR
import logging
...
def get_ssh_keys(client):
    ...
    logging.info(f"Found {len(ssh_keys)} SSH keys")
    return ssh_keys
...
def create_droplet(
    ...
    try:
        ...
        print(f"Attempting to create Droplet '{droplet_name}' in region '{region}' image '{image}' and size '{size}'...")
